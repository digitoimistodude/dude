 ...
 * @Last Modified time: 2022-10-04 10:53:05
 * @package dude
 */
...
function render_acf_block( $block, $content = '', $is_preview = false, $post_id = 0 ) {
  ...
  \do_action( 'qm/debug', "Block {$block_slug} output started" ); // phpcs:ignore WordPress.NamingConventions
   ...
   */
  $content_hash = crc32( serialize( get_fields() ) ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions
  ...
  if ( ! $block_cache_enabled ) {
    \do_action( 'qm/debug', "Block {$block_slug} bypassed cache ({$cache_key})" ); // phpcs:ignore WordPress.NamingConventions
...
function load_acf_block_from_cache( $cache_key, $block_slug, $block_path, $block, $is_preview = false, $post_id = 0 ) {
  ...
  if ( $output ) {
    \do_action( 'qm/debug', "Block {$block_slug} served from cache ({$cache_key})" ); // phpcs:ignore WordPress.NamingConventions
    return $output;
  }
  ...
  \do_action( 'qm/debug', "Block {$block_slug} cached ({$cache_key})" ); // phpcs:ignore WordPress.NamingConventions
...
function load_acf_block( $block_path, $cache = false, $block = [], $is_preview = false, $post_id = 0 ) {
  ...
  if ( ! $is_preview ) {
    ...
    if ( $post_type && 'wp_block' !== $post_type && is_array( $block['post_types'] ) && ! in_array( $post_type, $block['post_types'] ) ) { // phpcs:ignore WordPress.PHP.StrictInArray
      return '';
    }
  }
  ...
  if ( ! \file_exists( $block_path ) ) {
    \do_action( 'qm/error', "Block file {$block_path} not found" ); // phpcs:ignore WordPress.NamingConventions
    return '';
  }
...
function acf_block_maybe_enable_cache( string $block_slug ) {
  ...
  if ( empty( THEME_SETTINGS ) || empty( THEME_SETTINGS['acf_blocks'] ) ) {
    \do_action( 'qm/debug', 'Blocks couldnt be found in theme settings' ); // phpcs:ignore WordPress.NamingConventions
    return apply_filters( 'air_acf_block_maybe_enable_cache', $enable_cache, null );
  }

  $block_key = array_search( $block_slug, array_column( THEME_SETTINGS['acf_blocks'], 'name' ) ); // phpcs:ignore WordPress.PHP.StrictInArray
  if ( false === $block_key ) {
    \do_action( 'qm/debug', "Block {$block_slug} settings couldn't be found in theme settings" ); // phpcs:ignore WordPress.NamingConventions
