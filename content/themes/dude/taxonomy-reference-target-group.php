...
<main class="site-main archive-reference tax-reference-target-group">
      ...
      <div class="archive-head">
        ...
        <?php foreach ( $target_groups as $key => $label ) : ?>
          <button
            ...
            <?php if ( $key !== 'all' && $key !== $current_term->slug ) : ?>
  ...
  if ( ! empty( $reference_ids ) ) {
    ...
    echo '<!-- Found ' . count( $reference_ids ) . ' references in ' . $current_term->name . ' -->';
    ...
    echo '<div class="container"><p>Ei referenssejä tässä toimialassa. Varmista että olet lisännyt referenssejä "' . $current_term->name . '" toimialaan WordPress-hallinnassa.</p></div>';
  } ?>
