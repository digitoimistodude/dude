 ...
 * @Last Modified time: 2022-06-22 19:57:56
 */
// Dependencies
...
function watchFiles(done) {
  // Init BrowserSync
  bs.init(config.browsersync.src, config.browsersync.win);
  ...
  const devstyles = watch(
    config.styles.watch.development,
    series('devstyles')
  ...
  const prodstyles = watch(
    config.styles.watch.production,
    series('prodstyles')
  );
  ...
  // PHP
  const php = watch(config.php.watch, series('phpcswin'), bs.reload);
